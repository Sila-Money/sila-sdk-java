...
public class ResponseUtil {
    ...
    public static ApiResponse prepareResponse(HttpResponse<?> response, String msg) {
        ...
        case "link_account_refactored":
            com.silamoney.clientrefactored.endpoints.accounts.linkaccount.LinkAccountResponse linkAccountRResponse = (com.silamoney.clientrefactored.endpoints.accounts.linkaccount.LinkAccountResponse) Serialization
                    .deserialize(response.body().toString(),
                            com.silamoney.clientrefactored.endpoints.accounts.linkaccount.LinkAccountResponse.class);

            return new ApiResponse(statusCode, response.headers().map(), linkAccountRResponse,
                    linkAccountRResponse.isSuccess());
        case "get_transactions_refactored":
            com.silamoney.clientrefactored.endpoints.transactions.gettransactions.GetTransactionsResponse getTransactionsRResponse = (com.silamoney.clientrefactored.endpoints.transactions.gettransactions.GetTransactionsResponse) Serialization
                    .deserialize(response.body().toString(),
                            com.silamoney.clientrefactored.endpoints.transactions.gettransactions.GetTransactionsResponse.class);

            return new ApiResponse(statusCode, response.headers().map(), getTransactionsRResponse,
                    getTransactionsRResponse.isSuccess());
        case "request_kyc_refactored":
            RequestKycResponse requestKycResponse = (RequestKycResponse) Serialization
                    .deserialize(response.body().toString(), RequestKycResponse.class);

            return new ApiResponse(statusCode, response.headers().map(), requestKycResponse,
                    requestKycResponse.isSuccess());
        case "check_kyc_refactored":
            CheckKycResponse checkKycResponse = (CheckKycResponse) Serialization.deserialize(response.body().toString(),
                    CheckKycResponse.class);

            return new ApiResponse(statusCode, response.headers().map(), checkKycResponse,
                    checkKycResponse.isSuccess());
        ...
        case "get_accounts_refactored":
            List<com.silamoney.clientrefactored.domain.Account> accounts = (List<com.silamoney.clientrefactored.domain.Account>) JsonUtils
                    .deserialize(response.body().toString(), ArrayList.class);
            GetAccountsResponse getAccountsResponse = new GetAccountsResponse(accounts);

            return new ApiResponse(statusCode, response.headers().map(), getAccountsResponse, true);
